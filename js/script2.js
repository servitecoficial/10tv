...
function playChannel(ch) {
    ...
    // arrancamos en mute para garantizar el autoplay; después desactivamos cuando la reproducción comience
    pN.muted = true;
    pN.autoplay = true;
    ...
    if (ch.type === "yt") {
        ...
        pY.src = `https://www.youtube.com/embed/${ch.id}?autoplay=1&mute=1&controls=0&rel=0&enablejsapi=1`;
        // intentar desmutear el iframe después de unos instantes
        ...
        if (Hls.isSupported()) {
            ...
            hls.on(Hls.Events.MANIFEST_PARSED, () => {
                // INTENTO DE PLAY CON PROMESA (inicia mute)
                let playPromise = pN.play();
                if (playPromise !== undefined) {
                    playPromise.then(() => {
                        // la reproducción comenzó, ahora sí podemos desmutear
                        pN.muted = false;
                        pN.volume = 1.0;
            ...
            pN.src = ch.url;
            // mismo comportamiento: reproducción mute y luego desmute
            pN.muted = true;
            const fb = pN.play();
            if (fb !== undefined) {
                fb.then(() => {
...
function setupControls() {
    function unmuteAll() {
        ...
        const pY = document.getElementById('yt-player');
        if(pN) {
            if (pN.paused) pN.play();
            pN.muted = false;
            pN.volume = 1.0;
        }
        if(pY && pY.contentWindow) {
            // intenta mandar comando a la API de YouTube para desmutear
            pY.contentWindow.postMessage(JSON.stringify({
                event: 'command',
                func: 'unMute',
                args: []
            }), '*');
...
// también desbloqueamos el audio si el usuario hace clic o toca la pantalla
// (algunas plataformas no disparan keydown hasta más tarde)
